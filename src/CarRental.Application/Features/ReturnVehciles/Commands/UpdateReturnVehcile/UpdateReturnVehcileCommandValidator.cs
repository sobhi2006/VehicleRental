...
public class UpdateReturnVehicleCommandValidator : AbstractValidator<UpdateReturnVehicleCommand>
{
    ...
    private void ApplyCustomRules()
    {
        RuleFor(x => x.Id)
            ...
            {
                return await _returnVehicleService.ExistsByIdAsync(id, cancellationToken);
            })
            .WithMessage("ReturnVehicle not found.");
        ...
        RuleFor(x => x)
            ...
            {
                var alreadyReturned = await _returnVehicleService.ExistsByBookingIdExcludeSelfAsync(request.Id, request.BookingId, cancellationToken);
